...
import { Portfolio, buyStock, sellStock } from "@/utils/practicePortfolio";

interface PracticeTradeDialogProps {
  open: boolean;
  onClose: () => void;
  symbol: string;
  name: string;
  ...
  portfolio: Portfolio;
  practiceDate?: string;
  onTradeComplete: (portfolio: Portfolio, tradeTimestamp?: string) => void;
}
...
const PracticeTradeDialog = ({ 
  ...
  const handleBuy = () => {
    ...
    const result = buyStock(portfolio, symbol, name, quantityNum, currentPrice, practiceDate);
    
    if (result.success && result.portfolio) {
      onTradeComplete(result.portfolio, practiceDate);
      onClose();
      setQuantity("1");
    } else {
  ...
  const handleSell = () => {
    ...
    const result = sellStock(portfolio, symbol, quantityNum, currentPrice, practiceDate);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{symbol} - Practice Mode</DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          {/* Current Price */}
          <div className="p-4 bg-muted/50 rounded-lg">
            <p className="text-sm text-muted-foreground mb-1">Historical Price</p>
          ...
          <Tabs defaultValue="buy" className="w-full">
            ...
            <TabsContent value="buy" className="space-y-4 mt-4">
              ...
              >
                Buy {quantityNum} Share{quantityNum !== 1 ? 's' : ''} (Practice)
              </Button>
            </TabsContent>
            ...
            <TabsContent value="sell" className="space-y-4 mt-4">
              ...
              >
                Sell {quantityNum} Share{quantityNum !== 1 ? 's' : ''} (Practice)
              </Button>
            </TabsContent>
          </Tabs>
        </div>
