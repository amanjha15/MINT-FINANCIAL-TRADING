...
import { ArrowLeft, Trophy, TrendingUp, Target, Sparkles, Play, X } from "lucide-react";
...
const RapidTestGame = () => {
  ...
  const handleSubmitPrediction = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('rapid-test-prediction', {
        body: {
          query: prediction,
      ...
      if (output === "point" || output?.toLowerCase() === "point") {
        ...
        const messages = [
          `🎯 INCREDIBLE! ${newStreak} in a row! You're on fire! 🔥`,
          `💎 BRILLIANT! Keep that streak going! Score: ${newScore}`,
          `🌟 AMAZING! You're a trading genius! ${newStreak} streak!`,
          `🚀 SPECTACULAR! You're crushing it! Score: ${newScore}`,
          `⚡ PHENOMENAL! ${newStreak} correct predictions!`,
        ];
          ...
          <Card className="p-4 hover-scale transition-all">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p
                  className={`text-2xl font-bold text-chart-1 transition-all ${
