...
const Lessons = () => {
  ...
  const fetchLessons = async () => {
    const {
      data,
      error
    } = await supabase.from("lessons").select("*").order("order_index");
    if (error) {
      toast({
        title: "Error",
    ...
    const typedLessons: Lesson[] = (data || []).map(lesson => ({
      ...lesson,
      scenario_data: lesson.scenario_data as unknown as {
        scenarios: Scenario[];
      } | undefined,
  ...
  if (selectedLesson) {
    return <div className="min-h-screen bg-background py-12 px-4">
        <div className="container mx-auto max-w-6xl">
          ...
          {lessonMode === "content" && <StoryLesson title={selectedLesson.title} description={selectedLesson.description} content={selectedLesson.content} category={selectedLesson.category} difficulty={selectedLesson.difficulty} onComplete={proceedToScenarios} />}

          {/* Scenarios Mode */}
          {lessonMode === "scenarios" && selectedLesson.scenario_data?.scenarios && <>
              <ContentViewer title={selectedLesson.title} content={selectedLesson.content} category={selectedLesson.category} difficulty={selectedLesson.difficulty} description={selectedLesson.description} />
              
              {/* Scenario Progress Indicator */}
              {selectedLesson.scenario_data.scenarios.length > 1 && <Card className="p-4 mb-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-foreground">
                      Scenario {currentScenarioIndex + 1} of {selectedLesson.scenario_data.scenarios.length}
                    </span>
                    <div className="flex gap-2">
                      {selectedLesson.scenario_data.scenarios.map((_, idx) => <div key={idx} className={`h-2 w-8 rounded-full transition-all ${idx < currentScenarioIndex ? "bg-success" : idx === currentScenarioIndex ? "bg-primary animate-pulse" : "bg-muted"}`} />)}
                    </div>
                  </div>
                </Card>}
              
              <LessonScenario scenario={selectedLesson.scenario_data.scenarios[currentScenarioIndex]} onComplete={handleScenarioComplete} />
            </>}

          {/* Quiz Mode */}
          {lessonMode === "quiz" && selectedLesson.quiz_questions && <>
              <ContentViewer title={selectedLesson.title} content={selectedLesson.content} category={selectedLesson.category} difficulty={selectedLesson.difficulty} description={selectedLesson.description} />
              <LessonQuiz questions={selectedLesson.quiz_questions} onComplete={handleQuizComplete} />
            </>}

          {/* Practice Trading Mode */}
          {lessonMode === "practice" && selectedLesson.practice_stocks && <>
              <ContentViewer title={selectedLesson.title} content={selectedLesson.content} category={selectedLesson.category} difficulty={selectedLesson.difficulty} description={selectedLesson.description} />
              <PracticeTradingMode practiceStocks={selectedLesson.practice_stocks} practiceDate={selectedLesson.practice_start_date} lessonId={selectedLesson.id} onComplete={() => completeLesson(selectedLesson.id, selectedLesson.xp_reward)} />
            </>}
        </div>
      </div>;
  }
  return <div className="min-h-screen bg-background py-12 px-4 relative">
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          return <Card key={lesson.id} className={`p-6 hover:shadow-lg transition-all cursor-pointer bg-white ${completed ? "border-success" : ""}`} onClick={() => startLesson(lesson)}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${completed ? "bg-success/20" : "bg-primary/20"}`}>
                      {completed ? <CheckCircle2 className="w-6 h-6 text-success" /> : <BookOpen className="w-6 h-6 text-green-900" />}
                    </div>
                    <div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {completed && <Badge variant="default" className="bg-success">
                      Completed
                    </Badge>}
                </div>
              </Card>;
        ...
        {lessons.length === 0 && <Card className="p-12 text-center">
